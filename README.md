...
**Study Recommendation System** — мой pet-проект по разработке аналитической системы поддержки обучения.

Система использует историю решения учебных задач для оценки уровня освоения материала, определения тем, требующих повторения, и автоматического формирования персональных рекомендаций.

Проект объединяет проектирование базы данных PostgreSQL, ETL-процесс, аналитический слой, Recommendation Engine, Dashboard и собственный Task Bank.

При разработке проекта использовался генеративный ИИ как инструмент для обсуждения архитектурных решений, подготовки документации и анализа вариантов реализации. Все ключевые архитектурные решения, модель данных, алгоритмы Recommendation Engine и SQL-логика были разработаны и адаптированы мной.
...
Проект не проектировался как законченная рекомендательная система с самого начала.

Он развивался одновременно с накоплением собственной истории обучения.

Каждая новая решённая задача сначала добавлялась в систему вместе с информацией о попытке её решения.

Если задача относилась к уже существующему паттерну, она расширяла накопленную историю обучения.

Если же появлялась новая тема, для неё создавался новый паттерн в Dictionary.

После этого данный паттерн постепенно пополнялся тренировочными задачами в Task Bank.

Таким образом структура знаний проекта формировалась естественным образом по мере обучения, а Recommendation Engine автоматически начинал использовать новые паттерны без изменения своей логики.
...
- **Knowledge Base** — словарь паттернов и классификация учебного материала, который постепенно расширяется по мере появления новых тем;
...
- словарь паттернов определяет структуру знаний, используемую Recommendation Engine;
...
## Дальнейшее развитие модели знаний

В текущей версии проекта словарь паттернов (`dictionary`) формируется постепенно по мере появления новых учебных задач.

Такой подход хорошо подходит для накопления истории обучения, однако система анализирует только те темы, которые уже представлены в словаре.

В следующих версиях проекта планируется выделить независимую **Knowledge Map** — полную карту знаний предметной области.

Это позволит Recommendation Engine:

- оценивать полноту изучения дисциплины;
- выявлять темы, которые ещё не изучались;
- формировать рекомендации не только по повторению материала, но и по изучению новых тем.

При этом существующая архитектура проекта останется неизменной: Knowledge Map станет дополнительным источником информации для Recommendation Engine.
